...
| PROD promotion | `prod-promote.yml` re-verifies, re-scans and copies the approved digest, then edits the PROD overlay | GitHub-hosted with environment-scoped OIDC | One atomic desired-state PR auto-merges after required checks |
