...
check "expected_aws_account" {
  assert {
    ...
    error_message = "Refusing DEV bootstrap operation: caller does not match expected_aws_account_id."
  }
}
...
check "expected_aws_region" {
  assert {
    ...
    error_message = "Refusing DEV bootstrap operation: provider Region does not match aws_region."
  }
}
